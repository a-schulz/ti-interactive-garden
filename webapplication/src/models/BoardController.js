...
class BoardController {
  ...
  getRandomLightCondition() {
    const lightConditions = [ENVIRONMENT.SUNNY, ENVIRONMENT.PARTIAL_SHADE, ENVIRONMENT.SHADE];
  ...
  getRandomMoistureCondition() {
    const moistureConditions = [ENVIRONMENT.DRY, ENVIRONMENT.MOIST, ENVIRONMENT.WET];
    return moistureConditions[Math.floor(Math.random() * moistureConditions.length)];
  }
  ...
  checkEnvironment(plantId, row, col) {
    ...
    if (thrives) {
      return { status: 'happy', reason: `${plant.name} thrives in this environment!` };
    } else if (tolerates) {
      return { status: 'neutral', reason: `${plant.name} tolerates this environment.` };
    } else {
      return { status: 'unhappy', reason: `${plant.name} doesn't like this environment.` };
    }
  }
  ...
  checkNeighbors(plantId, row, col) {
    ...
    for (let r = Math.max(0, row-1); r <= Math.min(this.rows-1, row+1); r++) {
      for (let c = Math.max(0, col-1); c <= Math.min(this.cols-1, col+1); c++) {
        ...
        const relationship = plantDatabase.getRelationship(plantId, neighborPlantId);
  ...
  evaluatePlant(plantId) {
    ...
    // Check neighbors
    if (this.currentGameMode === GAME_MODE.NEIGHBORS_MODE || 
        this.currentGameMode === GAME_MODE.COMBINED_MODE) {
    ...
    if (this.currentGameMode === GAME_MODE.ENVIRONMENT_MODE) {
      ...
      if (environmentStatus.status === 'unhappy' || neighborStatus.status === 'unhappy') {
        status = 'unhappy';
        reason = 'Plant is unhappy with either environment or neighbors.';
      } else if (environmentStatus.status === 'happy' && neighborStatus.status === 'happy') {
        status = 'happy';
  ...
  getEnvironmentDescription(row, col) {
    ...
    const light = env & (ENVIRONMENT.SUNNY | ENVIRONMENT.PARTIAL_SHADE | ENVIRONMENT.SHADE);
    const moisture = env & (ENVIRONMENT.DRY | ENVIRONMENT.MOIST | ENVIRONMENT.WET);
    
    let lightDesc = 'Unknown';
    if (light === ENVIRONMENT.SUNNY) lightDesc = 'Sunny';
    else if (light === ENVIRONMENT.PARTIAL_SHADE) lightDesc = 'Partially Shaded';
